...
We will make a litle example, but do not reinvent the wheel, we have a lot of official and unofficial modules in the
Terraform Module Registry. Link: [Terraform Module Registry](https://www.terraform.io/docs/modules/usage.html).
...
Add the following variable to the new created `variables.tf` file:
...
Now we will use this litle module inside our project, for that we will create a `instances.tf` file with the following
content that is calling our own module:
...
We should also change the content of the `output.tf` file in our main project:
